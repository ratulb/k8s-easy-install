...
if [ ! -z "$masters" ]; then
  ...
  #echo '    --ignore-preflight-errors=all -v=6 &>/tmp/master-join-response' >>master-join-cluster.cmd
  echo '' >>master-join-cluster.cmd
  ...
  for me in $masters; do
    if [[ $cnt -eq 0 ]] || [[ "$me" = "$this_host_ip" ]]; then
      :
    else
      sudo -u $usr scp master-join-cluster.cmd $me:~/
    fi
    ((cnt++))
  done
